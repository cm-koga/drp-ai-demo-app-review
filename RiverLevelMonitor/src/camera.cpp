...
int8_t Camera::start_camera()
{
    ...
    int8_t overlay_buf_ret = -1;
    int8_t capture_buf_ret = -1;
    ...
    if (NULL == overlay_buf)
    {
        fprintf(stderr, "[ERROR] Failed to malloc the overlay_buf\n");
        goto err_end;
    }
    
    overlay_buf_ret = video_buffer_alloc_dmabuf(overlay_buf,WAYLANDBUF);
    if (-1 == overlay_buf_ret)
    {
        fprintf(stderr, "[ERROR] Failed to Allocate DMA buffer for the overlay_buf\n");
        goto err_end;
    }

    capture_buf = (camera_dma_buffer*)malloc(sizeof(capture_buf));
    if (NULL == capture_buf)
    {
        fprintf(stderr, "[ERROR] Failed to malloc the capture_buf\n");
        goto err_end;
    }
    
    capture_buf_ret = video_buffer_alloc_dmabuf(capture_buf,WAYLANDBUF);
    if (-1 == capture_buf_ret)
    {
...
err_end:
    ...
    if (0 == overlay_buf_ret)
    {
        video_buffer_free_dmabuf(overlay_buf);
    }
    
    if (0 == capture_buf_ret)
    {
        video_buffer_free_dmabuf(capture_buf);
    }
